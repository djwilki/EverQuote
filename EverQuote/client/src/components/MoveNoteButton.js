...
import { toggleMoveModal, toggleMoveNoteButton, toggleMoveTip } from '../store/ui';
import noteStyles from '../styles/note.module.css';

const MoveNoteButton = () => {
    const dispatch = useDispatch();
    const showMoveTip = useSelector(state => state.ui.showMoveTip)

    const handleMoveButton = () => {
        dispatch(toggleMoveNoteButton());
        dispatch(toggleMoveTip());
    };
    ...
    return (
        <div>
            <button className='moveNoteButton' onMouseEnter={handleMoveButton} onMouseLeave={handleMoveButton} onClick={handleMoveModal} style={{ padding: '4px 2px', cursor: 'pointer', verticalAlign: 'middle', borderRadius: '4px'}}>
            ...
            <div
            className={showMoveTip ? noteStyles.notebookTooltip : noteStyles.hidden}
            style={{ left: '50px'}}>
