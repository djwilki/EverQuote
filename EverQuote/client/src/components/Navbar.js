...
import { logout, setNoteList } from '../store/session';

function Navbar({ history, userId, selectedNotebookId }) {
  const dispatch = useDispatch();
  const notebooks = useSelector((state) =>
    Object.values(state.entities.notebooks)
  );
  const user = useSelector(
    (state) => state.entities.users[state.session.user_id]
  );
    ...
    <nav className={styles.navbar_container}>
      <div className={styles.navbar_content}>
        <div className={styles.top_section}>
          ...
          <div
            className={styles.note_button_content}
            onClick={handleNewNoteClick}
          >
            <svg
              className={styles.note_button_icon}
              fill="none"
            ...
            <button type="button" className={styles.new_note_button}>
              New Note
            </button>
          </div>
        </div>
        <div className={styles.navlinks}>
          ...
          >
            <svg
              className={`${styles.navlist_icon} ${styles.push_right}`}
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fillRule="evenodd"
                clipRule="evenodd"
                d="M7.665 4.5h8.75c.92 0 1.667.746 1.667 1.667v8.748h-3.334a.625.625 0 00-.625.624v3.958H7.665c-.92 0-1.667-.747-1.667-1.667V6.167c0-.92.747-1.667 1.667-1.667zm7.037 4.584a.625.625 0 100-1.25H9.298a.625.625 0 100 1.25h5.404zm.625 2.918c0 .345-.28.625-.625.625H9.298a.625.625 0 010-1.25h5.404c.345 0 .625.28.625.625zm-4.363 4.158a.625.625 0 100-1.25H9.298a.625.625 0 100 1.25h1.666z"
                fill="currentColor"
              ></path>
              <path
                d="M15.373 16.164h2.157l-2.107 2.693-.05.06v-2.753z"
                fill="currentColor"
              ></path>
            </svg>
            <div className={styles.navlink_text}>All Notes</div>
          </NavLink>
          <NavLink activeClassName={styles.active} exact to="/notebooks">
            ...
            <svg
              className={styles.navlist_icon_notebook}
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M8.035 4.5H5.958v15h2.077v-15z"
                fill="currentColor"
              ></path>
              <path
                fillRule="evenodd"
            ...
            <div className={styles.navlink_text}>Notebooks</div>
          </NavLink>
          <div className={styles.expanded_list_items}>
            {listNotebooks
              ? notebooks.map((ele) => {
                  return <NotebookSelect notebook={ele} />;
                })
              : ''}
          </div>
          <NavLink activeClassName={styles.active} exact to="/trash">
            ...
            <div className={styles.navlink_text}>Trash</div>
          </NavLink>
          <div className={styles.extra_navlinks}>
            <NavLink activeClassName={styles.active_extra} exact to="/about">
              <svg
                className={`${styles.navlist_icon} ${styles.extra_navlist_icon} ${styles.push_right}`}
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 24 24"
              >
                <path
                  id="270a"
                  d="M19.333 15.333a.667.667 0 01-.666-.666v-.477A7.321 7.321 0 0016.803 12h-6.47a2.333 2.333 0 000 4.667h1.334a3 3 0 003-3c0-.232-.039-.452-.092-.667h1.304c.071.323.121.655.121 1a4.667 4.667 0 01-4.667 4.667h-.666a4.667 4.667 0 110-9.334h6.112a7.312 7.312 0 001.888-2.21v-.456a.667.667 0 011.333 0v8a.667.667 0 01-.667.666zM6.091 10.667h-.758a.667.667 0 01-1.333 0V9.333a.667.667 0 011.333 0h2.122a5.694 5.694 0 00-1.364 1.334z"
                  fill="#ccc"
                ></path>
              </svg>
              <div className={styles.navlink_text}>About This Project</div>
            </NavLink>
