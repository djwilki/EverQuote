  ...
  return (
    <div className={noteStyles.editorTopSection}>
      <div className={noteStyles.editorTopLeftContainer}>
        ...
        <span className={noteStyles.updatedText}>
          {activeNote && activeNote.updated_at
            ? `Last updated ${genUpdatedAt(activeNote.updated_at)}`
            : ''}
        </span>
      </div>
...
const mapStateToProps = (state, ownProps) => {
  ...
  const activeNotebook = activeNote
    ? state.entities.notebooks[activeNote.notebookId]
    : null;
  return {
    activeNote,
    activeNotebook,
